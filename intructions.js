...
class intructions extends Phaser.Scene {
  ...
  create() {
    ...
    this.terrainLayer = map.createLayer("terrain", Terrain, tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.mainTerrainLayer = map.createLayer("mainTerrain", MainTerrain).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.mainCollideLayer = map.createLayer("mainCollide", MainCollide).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.mainOverlayLayer = map.createLayer("mainOverlay", MainOverlay).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.streetLayer = map.createLayer("street", Street).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.streetCollideLayer = map.createLayer("streetCollide", StreetCollide).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.streetOverlayLayer = map.createLayer("streetOverlay", StreetOverlay).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.buildingsLayer = map.createLayer("buildings", Buildings).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.mainTreeLayer = map.createLayer("mainTree", MainTree).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.mainTreeOverlayLayer = map.createLayer("mainTreeOverlay", MainTreeOverlay).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.treeLayer = map.createLayer("tree", Tree).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.buildingsOverlayLayer = map.createLayer("buildingsOverlay", BuildingsOverlay).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.fenceLayer = map.createLayer("fence", Fence).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    this.treeOverlayLayer = map.createLayer("treeOverlay", TreeOverlay).setPosition(tilemapX, tilemapY).setScale(mapScale).setVisible(t);
    //
    ...
    //characters//
    this.player = this.add.sprite(800, 830, "player").setScale(0.2).setVisible(true);
    this.pnpc = this.physics.add.sprite(600, 500, "npc").play("fpNPC").setScale(0.2).setVisible(false);
    //elaine models//
    this.elaine = this.add.image(250, 1150, "elaineP").setScale(0.6).setVisible(false);
    this.elaineL = this.add.image(250, 1150, "elaineL").setScale(0.6).setVisible(false);
    this.elaineS = this.add.image(250, 1150, "elaineS").setScale(0.6).setVisible(false);
    this.elaineSL = this.add.image(250, 1150, "elaineSL").setScale(0.6).setVisible(false);
    //
    //press spacebar notice//
    this.start = this.add.image(800, 1100, "pressToContinue").setScale(0.24).setOrigin(0.5);
    this.fade();
  ...
  npcTrigger() {
    if (space == 3) {
      this.npcMove()
      this.timedEvent = this.time.addEvent({delay: 2000, callback: this.npcMove, callbackScope: this, loop: true});
    }
  }
