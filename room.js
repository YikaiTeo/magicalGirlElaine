class room extends Phaser.Scene {
  ...
  create() {
    ...
    this.floorLayer = map.createLayer("floor", Floor, tilemapX, tilemapY).setScale(mapScale);
    this.wallLayer = map.createLayer("wall", Wall, tilemapX, tilemapY).setScale(mapScale);
    //another building layer is above the player, scroll down//
    this.furnitureLayer = map.createLayer("furniture", Furniture, tilemapX, tilemapY).setScale(mapScale);
    this.furnitureExtraLayer = map.createLayer("furnitureExtra", FurnitureExtra, tilemapX, tilemapY).setScale(mapScale);
    this.stairsLayer = map.createLayer("stairs", Stairs, tilemapX, tilemapY).setScale(mapScale);
    ...
    //player//
    var elaineScale = 0.2;
    this.player = this.physics.add.sprite(this.player.x, this.player.y, "elaine").setScale(elaineScale).setSize(250, 370);
    //
    //layers that overlay the players//
    this.furnitureOverlayLayer = map.createLayer("furnitureOverlay", FurnitureOverlay, tilemapX, tilemapY).setScale(mapScale);
  ...
  update() {
    ...
    if (this.player.x < 522 &&
      ...
      this.scene.stop("room");
      this.playerPosition = {}
      this.playerPosition.x = 2360;
      this.playerPosition.y = 1887;
